...
## Обработка таблиц      //!!!
						...
						if (is_array( $this->data[ $var ] )) {
                            ...
                            // $preg = '/\<w:tr.*' . $var . '.*?\<\/w:tr\>/';
                			preg_match( '/\<w:tr.*' . $var . '.*?\<\/w:tr\>/', $doc, $tmp1 );
                            
                            preg_match( '/^.*\<w:tr /', $tmp1[0], $tmp2 );
                            $tmp2 = preg_replace( '/^.*\<w:tr /', '<w:tr ', $tmp1[0] ); 

                            $arr = array();
                            foreach ( $this->data[$var] as $key => $item ) {
                                //$tmp2 = str_replace( $v, $this->_esc( $item ), $tmp2 );
                                $tmp3 = $tmp2;
                                if ( preg_match_all( '/\{[^}]+\}/', $tmp2, $m2 ) )
                                    foreach( $m2[0] as $v2 ) {
					                   $var2 = preg_replace('/[\s\{\}]/','', strip_tags( $v2 ));
                                       $tmp3 = str_replace( $v2, $this->_esc( $this->data[$var2][$key] ), $tmp3 );
                                    }
                                 $arr[] = $tmp3;
                            } 
                            
                            $doc = str_replace( $tmp2, implode( '', $arr ), $doc );
...
//			$bE = ($aP['GPF'] && 0x0001) ? TRUE : FALSE;
	...
	public function getEntryData( $name ) {
		$dir = dirname( $name );
		$name = basename( $name );
		foreach( (array)$this->package['entries'] as $entry)
			if ( $entry['path'] == $dir && $entry['name'] == $name)
				return $entry['data'];
		$this->error('Unknown format');
		return false;
